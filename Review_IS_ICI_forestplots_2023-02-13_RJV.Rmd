...
irAE <- dot_base +
  geom_point(aes(color = irAE_homogeneous), size = pointsize, shape = 16
  ) + geom_text(aes(label = irAE_homogeneous), fontface = "bold"
  ) + labs(title = "irAE type"
  )

indication <- dot_base +
  geom_point(aes(color = indication_IS_irAE), size = pointsize, shape = 16
  ) + geom_text(aes(label =indication_IS_irAE), fontface = "bold"
  ) + labs(title = "indication=irAE"
  )
...
```{r echo = T, eval = T, warning = T, error=T, message=T}
forrest_GC_OS <- ggarrange(p,HR,n,dose,ICI,tumour,irAE,indication,immortal_time_bias, nrow = 1, widths = c(8,2,1,2,1,1,1,1,1)
          # ,labels = c("\nOverall Survival", "\nHR (95% CI)", "\nn", "dose\ncut-off", "\nICI", "tumour\n", "\nirAE", "indication\n", "\nimmortal time")
...
forrest_GC_PFS <- ggarrange(p,HR,n,dose,ICI,tumour,irAE,indication,immortal_time_bias, nrow = 1, widths = c(8,2,1,2,1,1,1,1,1)
...
forrest_2LIS_OS <- ggarrange(p,HR,n,dose,ICI,tumour,irAE,indication,immortal_time_bias, nrow = 1, widths = c(8,2,1,3,1,1,1,1,1)
...
forrest_2LIS_PFS <- ggarrange(p,HR,n,dose,ICI,tumour,irAE,indication,immortal_time_bias, nrow = 1, widths = c(8,2,1,3,1,1,1,1,1)
                            # ,labels = c("\nProgression Free Survival", "\nHR (95% CI)", "\nn", "dose\ncut-off", "\nICI", "tumour\n", "\nindication", "immortal time\n")
                            # ,label.x = 0.5, hjust = 0.5, label.y = 1.1
                            # ,font.label = list(size =textsize, face = "bold")
) + theme(plot.margin = unit(c(0.5,0.4,0,0), units = "strwidth", data = "immortal time")
...
ggsave("review_IS_ICI_forrestplot_2L.jpg"
       , plot = p_2L_total
       , width = 13.5, height = 4.8, units = "in"
       , dpi = 1200
       )
```
